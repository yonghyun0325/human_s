...
<body>
    ...
    <div class="order-container">
        ...
        <div class="main-content">
            <div class="info-box">
                <p><span class="highlight">${user.userName}</span>님이 쇼핑몰에서 판매한 내역입니다.</p>
            </div>
            ...
            <table class="order-table">
                ...
                <tbody>
			        <c:if test="${not empty sellList}">
			            ...
			            <c:forEach var="order" items="${sellList}">
			                <tr>
			                ...
			                <c:set var="totalPrice" value="${ totalPrice + order.orPayAmount }" />
			            </c:forEach>
			            	<tr>
			            		<td colspan="5" style="text-align: right; padding-right: 2%;">총 금액 : ${ totalPrice }원</td>
			            	</tr>
			        </c:if>
			        <c:if test="${empty sellList}">
			            <tr>
			                <td colspan="5" class="no-data">판매내역이 없습니다.</td>
			            </tr>
			        </c:if>
			    </tbody>
            </table>
            ...
            <div class="notice">
                ...
                <p>※ 판매현황의 조회 버튼을 클릭하시면, 해당 주문의 배송 현황을 한눈에 확인하실 수 있습니다.</p>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.search-btn').on('click', function() {
                ...
                const today = $('#endDate').val();   // 종료 날짜 값
                ...
                // 서버로 GET 요청 보내기
                const url = "/hms/orderdetails/sellSearch.do?startDate=" + startDate + "&endDate=" + endDate.toISOString().split('T')[0];
