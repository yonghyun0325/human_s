...
//농사로 지역특산물 목록 조회
public class NsrLocalSpcprdApiExplorer {
    public static <T extends Object> T getApiJsonData( String serviceKey, String srcUrl, String pageNo, String numOfRows,
        ...
        urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode(numOfRows, "UTF-8")); /* 한페이지개수 */
        ...
        conn.setRequestProperty("Content-type", "application/xml"); //클라이언트가 서버로부터 수신하고 싶은 데이터 형식 지정
        ...
        //서버에서 받은 데이터를 Jackson API를 이용해서 자바DTO에 세팅하기 -> XML 데이터를 JSON으로 변환 및 DTO 매핑
        XmlMapper xmlMapper = new XmlMapper();
        T data = xmlMapper.readValue(sb.toString(), vo);
